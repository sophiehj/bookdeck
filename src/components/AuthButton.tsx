    ...
    <div className="flex flex-col items-end gap-1">
      <div className="flex items-center gap-1.5">
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white border border-[#E5E7EB] text-[#2D2D2D] text-xs font-medium hover:bg-gray-50 transition-colors shadow-sm disabled:opacity-60 cursor-pointer"
        >
          <GoogleIcon />
        ...
        <button
          ...
          disabled={loading !== null}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#FEE500] text-[#3C1E1E] text-xs font-medium hover:bg-[#f5dc00] transition-colors shadow-sm disabled:opacity-60 cursor-pointer"
        >
          <KakaoIcon />
          <span>{loading === 'kakao' ? '...' : '카카오'}</span>
        </button>
      </div>
      {error && (
